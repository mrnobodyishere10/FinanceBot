...
// Menggunakan GITHUB_TOKEN yang di-pass dari Environment Variables Actions
...
async function applyAutonomousUpdates(aiRes, owner, repo, branch, issueNumber) {
  ...
  const hasFileStart = aiRes.includes("###---AUTONOMOUS_FILE_START---###");
  const hasShellStart = aiRes.includes("###---SHELL_EXEC_START---###");

  if (hasFileStart || hasShellStart) {
...
export async function main() {
  ...
  const githubComment = eventData.comment?.body || eventData.pull_request?.body |
| "";
